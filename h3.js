...
function render() {
  ...
  if (lineStart && tempLineEnd && !lineEnd) {
    drawLine([lineStart[0], lineStart[1], tempLineEnd[0], tempLineEnd[1]], [0.5,0.5,0.5,1]);
  }

  if (lineStart && lineEnd) {
    drawLine([lineStart[0], lineStart[1], lineEnd[0], lineEnd[1]], [0,0,1,1]);
  }
  ...
  info += `Line: ${lineStart && lineEnd ? `(${lineStart[0].toFixed(2)},${lineStart[1].toFixed(2)}) ~ (${lineEnd[0].toFixed(2)},${lineEnd[1].toFixed(2)})` : "입력 대기"}<br>`;
...
canvas.addEventListener("mousedown", e => {
  if (mode === "circle") {
    ...
    if (!isLineDrawing) {
      ...
      lineEnd = p;
      isLineDrawing = false;
      intersectionPoints = computeIntersection(
...
canvas.addEventListener("mousemove", e => {
  if (mode === "circle" && isDragging) {
    ...
    render();
  } else if (mode === "line" && isLineDrawing) {
    tempLineEnd = toNDC(e.offsetX, e.offsetY);
    render();
  }
});
