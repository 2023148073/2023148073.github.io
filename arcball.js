...
export class Arcball {
  constructor(canvas, distance=5.0, sensitivity={rotation:1.0, zoom:0.001}){
    ...
    this.dragging=false; this.lastMouseX=0; this.lastMouseY=0;
  ...
  getArcballVector(x,y){
    ...
    const ny = (cy - y)/cy;
    const d = nx*nx+ny*ny;
    const nz = d<=1.0 ? Math.sqrt(1.0-d) : 0.0;
    const v = vec3.fromValues(nx,ny,nz);
    vec3.normalize(v,v); return v;
  }
  ...
  onMouseMove(e){
    ...
    const ang = Math.acos(Math.min(1.0, vec3.dot(va,vb))) * this.rotationSensitivity;
    const axis = vec3.create(); vec3.cross(axis, vb, va); vec3.normalize(axis, axis);
    const dq = quat.create(); quat.setAxisAngle(dq, axis, ang);
  ...
  getViewMatrix(){
    ...
    const V = mat4.create(); mat4.lookAt(V, eye, this.target, this.up); return V;
  }
  getModelRotMatrix(){
    const M = mat4.create(); mat4.fromQuat(M, this.rotation); return M;
  }
  getViewCamDistanceMatrix(){
    const V = mat4.create(); mat4.lookAt(V, this.position, this.target, this.up); return V;
  }
  reset(){ this.rotation = quat.create(); this.position = vec3.fromValues(0,0,this.distance); this.target = vec3.fromValues(0,0,0); }
}
